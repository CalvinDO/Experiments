namespace Matrix {
    ...
    export class Matrix4D {
        ...
        static readonly identity = new Matrix4D().setIdentity()

        at(index: number): number {
            return this.values[index]
        }

        init(values: number[]): Matrix4D {
            for (let i = 0; i < 16; i++) {
        ...
        copy(dest?: Matrix4D): Matrix4D {
            if (!dest) { dest = new Matrix4D() }
        ...
        equals(matrix: Matrix4D, threshold = this.epsilon): boolean {
            for (let i = 0; i < 16; i++) {
        ...
        multiplyVector3D(vector: Vector3D): Vector3D {
            const x = vector.x
            const y = vector.y
            const z = vector.z
        ...
        multiplyVector4D(vector: Vector4D, dest?: Vector4D): Vector4D {
            if (!dest) { dest = new Vector4D() }
        ...
        translate(vector: Vector3D): Matrix4D {
            const x = vector.x
            const y = vector.y
            const z = vector.z
        ...
        scale(vector: Vector3D): Matrix4D {
            const x = vector.x
            const y = vector.y
            const z = vector.z
        ...
        rotate(angle: number, axis: Vector3D): Matrix4D {
            let x = axis.x
            let y = axis.y
            let z = axis.z
        ...
        static frustum(left: number, right: number, bottom: number, top: number, near: number, far: number): Matrix4D {
            const rl = (right - left)
            const tb = (top - bottom)
            const fn = (far - near)
        ...
        static perspective(fov: number, aspect: number, near: number, far: number): Matrix4D {
            const top = near * Math.tan(fov * Math.PI / 360.0)
            const right = top * aspect

            return Matrix4D.frustum(-right, right, -top, top, near, far)
        }

        static orthographic(left: number, right: number, bottom: number, top: number, near: number, far: number): Matrix4D {
            const rl = (right - left)
            const tb = (top - bottom)
            const fn = (far - near)
        ...
        static lookAt(position: Vector3D, target: Vector3D, up: Vector3D = Vector3D.up): Matrix4D {
            if (position.equals(target)) {
                return this.identity
            }

            const z = Vector3D.difference(position, target).normalize()

            const x = Vector3D.cross(up, z).normalize()
            const y = Vector3D.cross(z, x).normalize()
            ...
            return new Matrix4D([
                ...
                -Vector3D.dot(x, position),
                -Vector3D.dot(y, position),
                -Vector3D.dot(z, position),
                1,
            ])
        }

        static product(m1: Matrix4D, m2: Matrix4D, result: Matrix4D): Matrix4D {
            const a00 = m1.at(0)
            const a01 = m1.at(1)
            const a02 = m1.at(2)
            const a03 = m1.at(3)
