...
function project(_projMatrix: Matrix4D, _viewMatrix: Matrix4D, _pos2D: Vector2D): Vector3D {
    let projInv: Matrix4D = _projMatrix.inverse();
    let viewInv: Matrix4D = _viewMatrix.inverse();
    ...
    let result: Vector3D = projInv.multiplyVector3D(viewInv.multiplyVector3D(new Vector3D([_pos2D.x, _pos2D.y, 0])));
